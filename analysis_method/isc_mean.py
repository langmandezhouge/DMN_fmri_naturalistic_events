...
isc_maps = {}
for task_name in all_task_names:
    isc_maps[task_name] = isc(bold[task_name], pairwise=False,summary_statistic='mean')
    
    
    # Make the ISC output a volume
    isc_vol = np.zeros(brain_nii.shape)
    # Map the ISC data for the first participant into brain space
    isc_vol[coords] = isc_maps[task_name]
    ...
    ax.set_title('ISC map for subjects mean, task = {}'.format(task_name))
    plt.show()
    ...
    # plot
    title_text = ('Left Avg ISC map, {} for sub-mean'.format(task_name))

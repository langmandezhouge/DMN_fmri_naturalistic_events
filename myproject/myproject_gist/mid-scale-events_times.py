...
for i in os.listdir(files):
    ...
    with open(filename2, "r", encoding="gb18030") as file:
        ...
        for line in lines:
            list2.append([line[0].replace("’", "'").replace("\xf1", "n"),
                          None if len(line) < 3 or len(line[2]) == 0 else line[2],
                          None if len(line) < 4 or len(line[3]) == 0 else line[3]])
    begin = None
    endin = None
    count = -1
    list4 = []
    ...
    for j in range(len(list2)):
        if list1[j][0] == list2[j][0]:
            if count != list1[j][1]:
                if count >= 0:
                    if begin is not None and endin is not None:
                        ...
                        if float(endin) - float(begin) > 2.0 and endin_TR > begin_TR:
            ...
            output1 = path + "final_times/"
            if not os.path.exists(output1):
                os.makedirs(output1)
            np.save(os.path.join(output1, i), list4)

            output2 = path + "final_events_line/"
            if not os.path.exists(output2):
                os.makedirs(output2)
            np.save(os.path.join(output2, i), data)
